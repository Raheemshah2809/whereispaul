        ...
        if (posts_div) {
            ...
            card.innerHTML = `
                <div class="card" data-post-id="${key}" data-aos="fade-up" data-aos-duration="1500">
                    <h4>Uploader: <a href="mailto:${encodeURIComponent(value.userEmail)}
                    ?subject=${encodeURIComponent(`Query For - Item Name: ${value.itemName ?? ''}`)} "_blank"> ${value.userEmail}</a></h4>
                    <h4>Upload Date: <br> ${value.timestamp} </h4>
                    <img class="img-fluid" src="${value.imageURL}"/>
                    <br>
                    <h4 class="title">Title: <br> ${value.itemName}</p>
                    <br>
